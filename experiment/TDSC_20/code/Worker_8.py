...
#数据
Task_data_8 = (4000,125,174,76,2000)
#Task_data_5 = (7190,127,190,75,1500)

#扰动
alpha_8 = [22,68,43,10,16]
...
def perb_data_gen():
    ...
    for m in range(TASK_NUM):
        perb_data_8.append(Task_data_8[m] - alpha_8[m])
    return perb_data_8
'''[7161, 60, 169, 36, 1482]
...
def pai_encrypt_data():
    ...
    for m in range(TASK_NUM):
        encrypted_data.append(pai.encipher(Task_data_8[m],paillier_public_key[0],paillier_public_key[1]))
...
def pai_encrypt_perb():
    ...
    pai = paillier_variant.Paillier()
    for m in range(TASK_NUM):
        encrypted_perbed.append(pai.encipher(alpha_8[m],paillier_public_key[0],paillier_public_key[1]))
...
'''
H_8j_1 = System_Params.hash_to_group(Identity_hash(U_ID[7]),q)
...
def main():
    ...
    #计算MAC
    z_8 = 12345677918
    s = str(aes_enc_perbed_data) + str(aes_enc_pai_enc_task_data) + str(aes_enc_pai_enc_perb) + str(aes_enc_pai_enc_perbpowsum) + str(H_8j)
    mac = System_Params.MAC(s,z_8)
    ...
    print(mac)
    '''
    return aes_enc_perbed_data,aes_enc_pai_enc_task_data,aes_enc_pai_enc_perb,aes_enc_pai_enc_perbpowsum,H_8j,mac
#main()