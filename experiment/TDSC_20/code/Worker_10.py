...
#数据
Task_data_10 = (4000,125,174,76,2000)
#Task_data_5 = (7190,127,190,75,1500)

#扰动
alpha_10 = [22,68,43,10,16]
...
def perb_data_gen():
    ...
    for m in range(TASK_NUM):
        perb_data_10.append(Task_data_10[m] - alpha_10[m])
    return perb_data_10
'''[7161, 60, 169, 36, 1482]
...
def pai_encrypt_data():
    ...
    for m in range(TASK_NUM):
        encrypted_data.append(pai.encipher(Task_data_10[m],paillier_public_key[0],paillier_public_key[1]))
...
def pai_encrypt_perb():
    ...
    pai = paillier_variant.Paillier()
    for m in range(TASK_NUM):
        encrypted_perbed.append(pai.encipher(alpha_10[m],paillier_public_key[0],paillier_public_key[1]))
...
'''
H_10j_1 = System_Params.hash_to_group(Identity_hash(U_ID[9]),q)
...
def main():
    ...
    #计算MAC
    z_10 = 123456779110
    s = str(aes_enc_perbed_data) + str(aes_enc_pai_enc_task_data) + str(aes_enc_pai_enc_perb) + str(aes_enc_pai_enc_perbpowsum) + str(H_10j)
    mac = System_Params.MAC(s,z_10)
    ...
    print(mac)
    '''
    return aes_enc_perbed_data,aes_enc_pai_enc_task_data,aes_enc_pai_enc_perb,aes_enc_pai_enc_perbpowsum,H_10j,mac
#main()