...
#数据
Task_data_5 = [4000,125,174,76,2000,
...
def perb_data_gen():
    ...
    for m in range(TASK_NUM):
        perb_data_5.append(Task_data_5[m] - alpha_5[m])
    return perb_data_5
'''[7161, 60, 169, 36, 1482]
...
def pai_encrypt_data():
    ...
    for m in range(TASK_NUM):
        encrypted_data.append(pai.encipher(Task_data_5[m],paillier_public_key[0],paillier_public_key[1]))
...
def pai_encrypt_perb():
    ...
    pai = paillier_variant.Paillier()
    for m in range(TASK_NUM):
        encrypted_perbed.append(pai.encipher(alpha_5[m],paillier_public_key[0],paillier_public_key[1]))
...
'''
H_5j_1 = System_Params.hash_to_group(Identity_hash(U_ID[4]),q)
...
def main():
    ...
    #计算MAC
    z_5 = 12345678912
    s = str(aes_enc_perbed_data) + str(aes_enc_pai_enc_task_data) + str(aes_enc_pai_enc_perb) + str(aes_enc_pai_enc_perbpowsum) + str(H_5j)
    mac = System_Params.MAC(s,z_5)
    ...
    print(mac)
    '''
    return aes_enc_perbed_data,aes_enc_pai_enc_task_data,aes_enc_pai_enc_perb,aes_enc_pai_enc_perbpowsum,H_5j,mac
#main()